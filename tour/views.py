...
import tour.recommend.recommend as r
...
def init(request):
    if request.method == 'GET':
        ...
        hot = View.objects.filter(city=u'成都').order_by('view_rate')[::-1]

        # 随机推荐
        rand = View.objects.order_by('?')[:10]

        # 猜你喜欢
        # guess = View.objects.order_by('view_rate')[:10]
        guess = View.objects.filter(view_desc__in = r.get_data(user_name=request.user))[:10]
        print("================", guess)
